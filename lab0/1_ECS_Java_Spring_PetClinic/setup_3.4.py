...
def docker_login_config():
    ...
    #.encode( 'utf-8')
    hostname = "https://"+ecr_client.get_authorization_token().get('authorizationData')[0]['proxyEndpoint'][8:]
    #.encode('utf-8')[8:]
    ...
    with open(filename, 'r+') as f:
        ...
        logger.info('Writing docker configuration as '+json.dumps(data))
        f.seek(0)
...
def setup(project_name='spring-petclinic-rest', service_list={'spring-petclinic-rest': '8080'}, region='us-west-2'):
    ...
    for service in service_list:
        ...
        containerDefinitions = [
            {
                'name': service,
                'image': repository_uri[list(service_list.keys()).index(service)][service] + ':latest',
                'essential': True,
                'portMappings': [
                    {
    ...
    for service in service_list:
        ...
        try:
            ...
            ecs_client.deregister_task_definition(
                taskDefinition=task_definition['taskDefinition']['family']+ ':' + str(
